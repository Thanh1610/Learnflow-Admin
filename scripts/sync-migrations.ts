...
const CLIENT_DIR = path.join(ADMIN_DIR, '..', 'Learnflow-Client');
...
function syncSchema() {
  ...
  console.log('✅ Schema synced to Client');
}
...
function syncMigrations() {
  ...
  if (fs.existsSync(lockFile)) {
    ...
    console.log('✅ migration_lock.toml synced');
  }
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      copiedCount++;
      console.log(`✅ Migration ${entry.name} synced`);
    }
  }

  if (copiedCount === 0) {
    console.log('ℹ️  No new migrations to sync');
  }
}
...
// Main execution
try {
  console.log('🔄 Syncing migrations from Admin to Client...\n');
  syncSchema();
  syncMigrations();
